...
public class AddIntervista extends JFrame implements ActionListener {
	...
	public AddIntervista(Controllore cntrl) {
		setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		
		setName("AddIntervista");
		setTitle("AddIntervista");
		controller = cntrl;
		setResizable(false);
       setBounds(10, 10, 400, 480);
		...
		btnAutore.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(autore.length()>0) {
					controller.intervistaDAO.inserisciautore(titolo, autore, btnAutore);
				} else {
		...
		btnRiferimenti.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(cita.length()>0  && controller.intervistaDAO.checkriferimento(titolo, cita, btnRiferimenti)) {
					controller.intervistaDAO.inserisciriferimento(titolo, cita, btnRiferimenti);
				} else {
		...
		btnInserisci2.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				String autore = txtAutore.getText();
				String cita = txtCita.getText();
				if (controller.intervistaDAO.checkautore( titolo,autore,btnInserisci2 )) {
		...
		btnInserisci.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
	           ...
	           if((controller.checktitolo(titolo, btnInserisci))&&
	            (controller.checknumpagint(numpag, btnInserisci)) &&
	          
	           (controller.checkdate(dins, datapubblicazione, btnInserisci))) {
	        	   System.out.println("ok");
	        	   controller.registraintervista(titolo, numpag, descrizione, dins, datapubblicazione, btnInserisci);
